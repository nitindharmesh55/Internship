...
const API_KEY = "5608a1f3ae40433aa1000836251502"; // Replace with your API key
...
const displayHourlyForecast = (hourlyData, data) => {
...
const getWeatherDetails = async (API_URL) => {
  ...
  try {
    ...
    currentWeatherDiv.querySelector(".description").innerText = `${description} | Humidity: ${humidity}% | Wind: ${windSpeed} km/h`;

    // Combine hourly data from today and tomorrow
    const combinedHourlyData = [...data.forecast?.forecastday[0]?.hour, ...data.forecast?.forecastday[1]?.hour];
    searchInput.value = data.location.name;
    displayHourlyForecast(combinedHourlyData, data);
  } catch (error) {
